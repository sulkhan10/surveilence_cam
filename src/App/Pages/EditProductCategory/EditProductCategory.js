...
class EditProductCategory extends Component {
    constructor(props) {
		...
		this.language = getLanguage(activeLanguage, 'editproductcategory');
		this.state = {
			productcategoryid: props.match.params.productcategoryid,
			productcategoryname: ''
		}
    }
	...
	checkData = () => {
	 ...
	 componentDidMount = () => {
		...
		axios.post(serverUrl+'productcategory_get_by_id.php', {
		...
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response) =>{
				...
				this.setState({productcategoryname : response.data.record.productcategoryname});
            })
            .catch((error) =>{
				this.props.doLoading();
                console.log(error);
                ...
                <div className="box-container">
					<table>
						<tr>
                            ...
                            <td><Input type="text" name="productcategoryname" id="productcategoryname" value={this.state.productcategoryname} onChange = {(event) => this.setState({ productcategoryname : event.target.value }) }/></td>
                        </tr>
                    </table>
                </div>
