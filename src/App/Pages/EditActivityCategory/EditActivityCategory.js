...
class EditActivityCategory extends Component {
    constructor(props) {
		super(props);
		this.globallang = getLanguage(activeLanguage, 'global');
		this.language = getLanguage(activeLanguage, 'editactivitycategory');
		this.state = {
	...
	checkData = () => {
		...
		if(activitycategoryname == null || activitycategoryname == '' || icon == null || icon == ''){
	 ...
	 componentDidMount = () => {
		...
		axios.post(serverUrl+'activitycategory_get_by_id.php', {
		...
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response) =>{
				...
				this.setState({activitycategoryname : response.data.record.activitycategoryname});
	...
	onSubmit = () => {
		...
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then( (response) =>{
				...
				this.props.history.push('/panel/listactivitycategory');
            })
            .catch( (error) =>{
				this.props.doLoading();
                console.log(error);
        ...
        return (
            <div>
                ...
                <div className="box-container">
					<table>
						<tr>
                            <td><Label for="activitycategoryname">{this.language.fieldname}</Label></td>
