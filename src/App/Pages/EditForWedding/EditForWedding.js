...
import './EditForWedding.style.css';
...
const customStyles = {
  ...
  class EditForWedding extends Component {
    constructor(props) {
		super(props);
		...
		this.state = {
			...
			forweddingid: props.match.params.forweddingid,
			for_weddingname: '',
	...
	onUploadImage = (result) => {
        this.setState({ forweddingpic: result });
    }
	...
	checkData = () => {
		const {for_weddingname} = this.state;
		const {forweddingcategoryid} = this.state;
		const {shortdesc} = this.state;
		const {fulldesc} = this.state;
		const {forweddingpic} = this.state;
		const {communityid} = this.state;
		const {isavailable} = this.state;
		
		if(for_weddingname == '' || forweddingcategoryid == 0 || shortdesc == '' || fulldesc == '' || communityid == 0 || forweddingpic == null){
			alert(this.language.validation);
			return false;
		}
	...
	removeInfo=(info)=>{
	 ...
	 componentDidMount = (forweddingCategoryShow, communityShow) => {
		...
		axios.post(serverUrl+'forwedding_get_by_id.php', {
            forweddingid: this.state.forweddingid
		}, 
		...
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response, forweddingcategoryid, communityid) =>{
				...
				let tmp = [];
				if(response.data.record.forweddingpic !== ""){
					tmp.push(response.data.record.forweddingpic);
				}
				
				this.setState({forweddingid : response.data.record.forweddingid});
				this.setState({for_weddingname : response.data.record.for_weddingname});
				...
				this.setState({forweddingpic : tmp});
	...
	onSubmit = () => {
		this.props.doLoading();
        axios.post(serverUrl+'forwedding_insert_update.php', {			
			forweddingid: this.state.forweddingid,
			for_weddingname: this.state.for_weddingname,
			forweddingcategoryid: this.state.forweddingcategoryid,
			shortdesc: this.state.shortdesc,
			fulldesc: this.state.fulldesc,
			forweddingpic: this.state.forweddingpic,
		...
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then( (response) =>{
				...
				if(response.data.status === "ok"){
					...
					this.props.history.push('/panel/listforwedding');
				}else{
        ...
        return (
            <div>
                ...
                <div className="box-container">
					<table>
						<tr>
                            <td><Label for="for_weddingname">Banquet Facilities Name</Label></td>
                            <td><Input type="text" name="for_weddingname" id="for_weddingname" placeholder="Input Banquet Facilities Name" value={this.state.for_weddingname} onChange = {(event) => this.setState({ for_weddingname : event.target.value }) }/></td>
                        </tr>
						<tr>
						&nbsp;
						</tr>
						...
						<tr>
                            <td><Label>{this.globallang.uploadpicture}</Label></td>
							<td>
							<PictureUploader onUpload={this.onUploadImage} picList = {this.state.forweddingpic} picLimit={1}></PictureUploader>
							</td>
                        </tr>
						<tr>
						&nbsp;
