...
class ListProductCategory extends Component {
    constructor(props) {
		...
		this.language = getLanguage(activeLanguage, 'listproductcategory');
		this.state = {
			tableData: [],
			filter:''
        }
	...
	doRowEdit = (row) => {
        console.log(row);
		this.props.history.push('/panel/editproductcategory/'+row.productcategoryid);
    }
		  ...
		  buttons: [
			{
				...
				onClick: (productcategoryid) => {
					...
					this.deleteProductCategory(productcategoryid)					
			  }
			},
			{
			  label: 'No',
	...
	addNew = () => {
		this.props.history.push('/panel/inputproductcategory');
	}
	
	doSearch = () => {
		this.props.doLoading();
		axios.post(serverUrl+'productcategory_list.php', {
	...
	deleteProductCategory = (productcategoryid) => {
		this.props.doLoading();
		axios.post(serverUrl+'productcategory_delete.php', {
