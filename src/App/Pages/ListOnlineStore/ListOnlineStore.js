...
class ListOnlineStore extends Component {
    constructor(props) {
        super(props);
		this.globallang = getLanguage(activeLanguage, 'global');
		this.language = getLanguage(activeLanguage, 'listonlinestore');
		this.state = {
			tableData: [],
			filter:'',
		...
		{
            Header: 'Online Store Picture',
			headerStyle: {fontWeight : 'bold'},
            accessor: 'onlinestorepic',
            style: { textAlign: "center"}
        },*/ 		
		{
	...
	doRowEdit = (row) => {
		this.props.history.push('/panel/editonlinestore/'+row.onlinestoreid);
    }
    ...
    doRowDelete = (row) => {
		confirmAlert({
		  ...
		  buttons: [
			{
				label: 'Yes',
				onClick: (onlinestoreid) => {
					var onlinestoreid = row.onlinestoreid;
					console.log(onlinestoreid);
					this.deleteOnlineStore(onlinestoreid)					
			  }
			},
			{
			  label: 'No',
	...
	addNew = () => {
		this.props.history.push('/panel/inputonlinestore');
	}
	
	doSearch = () => {
		this.props.doLoading();
		axios.post(serverUrl+'onlinestore_list.php', {
	...
	deleteOnlineStore = (onlinestoreid) => {
		this.props.doLoading();
		axios.post(serverUrl+'onlinestore_delete.php', {
...
export default ListOnlineStore;