...
class ParkIntroduction extends Component {
    constructor(props) {
		super(props);
		
		this.globallang = getLanguage(activeLanguage, 'global');
		this.language = getLanguage(activeLanguage, 'parkintroduction');
		this.state = {
			id: 0,
			title: '',
			description: '',
		...
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then( (response) =>{
				...
				if(response.data.status === "ok"){
					alert('OK');
                    //this.props.history.push('/panel/parkintroduction');
...
export default ParkIntroduction;