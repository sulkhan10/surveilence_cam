...
class ComplaintEdit extends Component {
    constructor(props) {
		...
		this.language = getLanguage(activeLanguage, 'detailcomplaintform');
		this.state = {
			userbillingid: props.match.params.userbillingid,
			userShow: [],
			phoneno: '',
			billingcategoryid: '',
			billingCategoryShow: [],
	...
	checkData = () => {
	 ...
	 componentDidMount = () => {
		...
		axios.post(serverUrl+'billing_get_by_id.php', {
		...
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response, onlinestorecategoryid, communityid) =>{
				...
				this.setState({userbillingid : response.data.record.userbillingid});
				this.setState({phoneno : response.data.record.phoneno});
				this.setState({billingcategoryid : response.data.record.billingcategoryid});
				this.setState({month : response.data.record.month});
				this.setState({year : response.data.record.year});
				this.setState({customerid : response.data.record.customerid});
				this.setState({amount : response.data.record.amount});
				this.setState({ispaid : response.data.record.ispaid===1?true:false});
