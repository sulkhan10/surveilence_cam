...
class EditMembershipFeePage extends Component {
  constructor(props) {
    super(props);
  ...
  componentDidMount = () => {
    ...
    axios
      .post(
        serverUrl + "membership_fee_get_by_id.php",
        {
      ...
      .then((response) => {
        this.props.doLoading();
        this.setState({ feeId: response.data.record.feeId });
        this.setState({ fee: response.data.record.fee });
      })
      .catch((error) => {
        this.props.doLoading();
        console.log(error);
...
export default EditMembershipFeePage;
