...
import './InputNonWedding.style.css';
...
class InputNonWedding extends Component {
    constructor(props) {
		super(props);
		...
		this.state = {
			...
			modalIsOpen: false,
			icon: [],
			nonweddingcategoryisavailable: false
		}
	...
	addRoomCategory = (nonweddingcategoryid) => {
		const {non_weddingcategoryname} = this.state;
		const {icon} = this.state;
		
		if(non_weddingcategoryname == null || non_weddingcategoryname == '' || icon == null){
			alert(this.language.validation);
			return false;
		}
		
		else {
			axios.post(serverUrl+'nonweddingcategory_insert_update.php', {			
                nonweddingcategoryid: this.state.nonweddingcategoryid,
                non_weddingcategoryname: this.state.non_weddingcategoryname,
			...
			{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
				.then( (response) =>{
					...
					this.closeModal();
					this.setState({nonweddingcategoryid: response.data.record});
					this.selectNonWeddingCategory();
	...
	nonweddingCategoryIsAvailableChecked (event) {
		let checked = event.target.checked;
		this.setState({nonweddingcategoryisavailable: checked});
	}
	 
	checkData = () => {
		const {non_weddingname} = this.state;
		const {nonweddingcategoryid} = this.state;
		...
		if(non_weddingname == '' || nonweddingcategoryid == 0 || shortdesc == '' || fulldesc == '' || communityid == 0 || nonweddingpic == null){
			alert(this.language.validation);
			return false;
		}
...
export default InputNonWedding;