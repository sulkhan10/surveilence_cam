...
class EditService extends Component {
    constructor(props) {
		...
		this.language = getLanguage(activeLanguage, 'editservice');
		this.state = {
	...
	removeInfo=(info)=>{
	 ...
	 componentDidMount = (serviceCategoryShow, companyShow, tagShow) => {
		...
		axios.post(serverUrl+'service_get_by_id.php', {
            serviceid: this.state.serviceid
		}, 
		
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response, servicecategoryid, companyid) =>{
				this.props.doLoading();
                console.log(response.data);
				let tmp = [];
				if(response.data.record.servicepic !== ""){
					tmp.push(response.data.record.servicepic);
				}
				this.setState({serviceid : response.data.record.serviceid});
				this.setState({servicename : response.data.record.servicename});
