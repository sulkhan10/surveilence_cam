...
class ListInvestment extends Component {
    constructor(props) {
        super(props);
		this.globallang = getLanguage(activeLanguage, 'global');
		this.language = getLanguage(activeLanguage, 'listinvestment');
		this.state = {
			tableData: [],
			filter:'',
		...
		{
            Header: 'Investment Picture',
			headerStyle: {fontWeight : 'bold'},
            accessor: 'investmentpic',
            style: { textAlign: "center"}
        },*/ 		
		{
	...
	doRowEdit = (row) => {
		this.props.history.push('/panel/editinvestment/'+row.investmentid);
    }
    ...
    doRowDelete = (row) => {
		confirmAlert({
		  ...
		  buttons: [
			{
				label: 'Yes',
				onClick: (investmentid) => {
					var investmentid = row.investmentid;
	...
	addNew = () => {
		this.props.history.push('/panel/inputinvestment');
	}
	
	doSearch = () => {
		this.props.doLoading();
		axios.post(serverUrl+'investment_list.php', {
	...
	deleteInvestment = (investmentid) => {
		this.props.doLoading();
		axios.post(serverUrl+'investment_delete.php', {
