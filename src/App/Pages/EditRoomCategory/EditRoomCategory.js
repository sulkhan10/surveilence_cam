...
class EditRoomCategory extends Component {
    constructor(props) {
		...
		this.language = getLanguage(activeLanguage, 'editroomcategory');
		this.state = {
			roomcategoryid: props.match.params.roomcategoryid,
			roomcategoryname: '',
			icon: [],
			isavailable: false
		}
	...
	checkData = () => {
	 ...
	 componentDidMount = () => {
		...
		axios.post(serverUrl+'roomcategory_get_by_id.php', {
		...
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response) =>{
				...
				this.setState({roomcategoryname : response.data.record.roomcategoryname});
