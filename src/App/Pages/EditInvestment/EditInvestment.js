...
class EditInvestment extends Component {
    constructor(props) {
		...
		this.language = getLanguage(activeLanguage, 'editinvestment');
		this.state = {
	...
	removeInfo=(info)=>{
	 ...
	 componentDidMount = (investmentCategoryShow, companyShow, tagShow) => {
		...
		axios.post(serverUrl+'investment_get_by_id.php', {
            investmentid: this.state.investmentid
		}, 
		
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response, investmentcategoryid, companyid) =>{
				this.props.doLoading();
                console.log(response.data);
				let tmp = [];
				if(response.data.record.investmentpic !== ""){
					tmp.push(response.data.record.investmentpic);
				}
				this.setState({investmentid : response.data.record.investmentid});
				this.setState({investmentname : response.data.record.investmentname});
				...
				this.setState({investmentpic : tmp});
