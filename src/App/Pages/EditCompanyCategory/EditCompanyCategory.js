...
class EditCompanyCategory extends Component {
    constructor(props) {
		super(props);
		this.globallang = getLanguage(activeLanguage, 'global');
		this.language = getLanguage(activeLanguage, 'editcompanycategory');
		this.state = {
			companycategoryid: props.match.params.companycategoryid,
			companycategoryname: ''
		}
    }
	...
	checkData = () => {
	 ...
	 componentDidMount = () => {
		...
		axios.post(serverUrl+'companycategory_get_by_id.php', {
		...
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response) =>{
				...
				this.setState({companycategoryname : response.data.record.companycategoryname});
            })
            .catch((error) =>{
				this.props.doLoading();
                console.log(error);
