...
class EditOnlineStore extends Component {
    constructor(props) {
		super(props);
		this.globallang = getLanguage(activeLanguage, 'global');
		this.language = getLanguage(activeLanguage, 'editonlinestore');
		this.state = {
			onlinestoreid: props.match.params.onlinestoreid,
			onlinestorename: '',
	...
	removeInfo=(info)=>{
	 ...
	 componentDidMount = (onlinestoreCategoryShow, communityShow, tagShow) => {
		this.selectOnlineStoreCategory();
		this.selectCommunity();
		this.selectTag();
		this.props.doLoading();
		axios.post(serverUrl+'onlinestore_get_by_id.php', {
		...
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response, onlinestorecategoryid, communityid) =>{
				...
				let tmp = [];
				if(response.data.record.onlinestorepic !== ""){
					tmp.push(response.data.record.onlinestorepic);
				}
				this.setState({onlinestoreid : response.data.record.onlinestoreid});
				...
				this.setState({onlinestorepic : tmp});
...
export default EditOnlineStore;