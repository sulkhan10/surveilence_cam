...
class ListServiceCenter extends Component {
    constructor(props) {
		super(props);
		this.reactTable = React.createRef();
		this.globallang = getLanguage(activeLanguage, 'global');
		this.language = getLanguage(activeLanguage, 'listservicecenter');
		this.state = {
			tableData: [],
			filter:'',
	...
	doRowEdit = (row) => {
		this.props.history.push('/panel/editservicecenter/'+row.servicecenterid);
    }
    ...
    doRowDelete = (row) => {
		confirmAlert({
		  ...
		  buttons: [
			{
				label: 'Yes',
				onClick: (servicecenterid) => {
					var servicecenterid = row.servicecenterid;
					this.deleteServiceCenter(servicecenterid)					
			  }
			},
			{
			  label: 'No',
	...
	addNew = () => {
		this.props.history.push('/panel/inputservicecenter');
	}
	
	doSearch = () => {
		this.props.doLoading();
		axios.post(serverUrl+'servicecenter_list.php', {
	...
	deleteServiceCenter = (servicecenterid) => {
		this.props.doLoading();
		axios.post(serverUrl+'servicecenter_delete.php', {
