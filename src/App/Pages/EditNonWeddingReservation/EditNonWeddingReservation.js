...
class EditNonWeddingReservation extends Component {
    constructor(props) {
		super(props);
		this.globallang = getLanguage(activeLanguage, 'global');
		this.language = getLanguage(activeLanguage, 'editnonweddingreservation');
		this.state = {
			nonweddingreservationid: props.match.params.nonweddingreservationid,
			userShow: [],
			phoneno: '',
			startdate: moment(),
			enddate: moment(),
	...
	selectFacilities = () =>{
		axios.post(serverUrl+'nonwedding_list.php', {filter: ''},
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then( (response) =>{
                this.setState({ nonweddingShow: response.data.records});
            })
            .catch( (error) =>{
                console.log(error);
				alert(error);
	...
	changeFacilities = (nonweddingid)=>{
        this.setState({nonweddingid: nonweddingid});
    }
	...
	checkIfExist=()=>{
	 ...
	 componentDidMount = () => {
		...
		axios.post(serverUrl+'nonweddingreservation_get_by_id.php', {
		...
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response, onlinestorecategoryid, communityid) =>{
				...
				this.setState({nonweddingreservationid : response.data.record.nonweddingreservationid});
				...
				this.setState({nonweddingname : response.data.record.nonweddingname});
				this.setState({duration : response.data.record.duration});
				this.setState({nonweddingprice : response.data.record.nonweddingprice});
            ...
            <div>
                ...
                <div className="box-container">
					<table>
						...
						<tr>
                            <td><Label for="nonweddingid"> Facilities Name</Label></td>
                            <td>
								<SelectMultiColumn width={200} value={this.state.nonweddingid} valueColumn={'nonweddingid'} showColumn={'non_weddingname'} columns={['non_weddingname']} data={this.state.nonweddingShow} onChange={this.changeFacilities} />
							</td>
                        </tr>
						<tr>
						&nbsp;
						...
						<tr>
                            ...
                            <td><Input type="text"  name="estimated" id="estimated" placeholder="Rp." value={convertToRupiah(this.state.nonweddingprice)} onChange = {(event) => this.setState({ weddingprice : event.target.value }) }/></td>
                        </tr>
						<tr>
						&nbsp;
						</tr>
...
export default EditNonWeddingReservation;