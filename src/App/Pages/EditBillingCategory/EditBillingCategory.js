...
class EditBillingCategory extends Component {
    constructor(props) {
		super(props);
		this.globallang = getLanguage(activeLanguage, 'global');
		this.language = getLanguage(activeLanguage, 'editbillingcategory');
		this.state = {
			billingcategoryid: props.match.params.billingcategoryid,
			billingcategoryname: '',
			icon: [],
			isavailable: false
		}
	...
	checkData = () => {
	 ...
	 componentDidMount = () => {
		...
		axios.post(serverUrl+'billingcategory_get_by_id.php', {
		...
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response) =>{
				...
				this.setState({billingcategoryname : response.data.record.billingcategoryname});
...
export default EditBillingCategory;