...
class EditEntertainment extends Component {
    constructor(props) {
		super(props);
		this.globallang = getLanguage(activeLanguage, 'global');
		this.language = getLanguage(activeLanguage, 'editentertainment');
		this.state = {
			entertainmentid: props.match.params.entertainmentid,
			link: '',
			image: [],
			isavailable: false
		}
	...
	checkData = () => {
	 ...
	 componentDidMount = () => {
		...
		axios.post(serverUrl+'entertainment_get_by_id.php', {
		...
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response) =>{
				...
				if(response.data.record.image !== ""){
					tmp.push(response.data.record.image);
				}
				this.setState({entertainmentid : response.data.record.entertainmentid});
