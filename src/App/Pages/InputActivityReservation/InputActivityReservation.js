...
class InputActivityReservation extends Component {
    constructor(props) {
		super(props);
		this.globallang = getLanguage(activeLanguage, 'global');
		this.language = getLanguage(activeLanguage, 'inputactivityreservation');
		this.state = {
	...
	checkData = () => {
		...
		if(phoneno == '' || activityid == 0 || price == null || communityid == 0){
	...
	onSubmit = () => {
		...
		checkin = yearin+"-"+(monthin+1)+"-"+datein+" "+starttime+":00";
		checkout = yearout+"-"+(monthout+1)+"-"+dateout+" "+endtime+":00";
...
export default InputActivityReservation;