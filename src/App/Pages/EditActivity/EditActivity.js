...
class EditActivity extends Component {
    constructor(props) {
		...
		this.language = getLanguage(activeLanguage, 'editactivity');
		this.state = {
	...
	selectActivityCategory = (activityCategoryShow) =>{
	...
	removeInfo=(info)=>{
	 ...
	 componentDidMount = (activityCategoryShow, communityShow) => {
		this.selectActivityCategory(activityCategoryShow);
		this.selectCommunity(communityShow);
		this.props.doLoading();
		axios.post(serverUrl+'activity_get_by_id.php', {
            activityid: this.state.activityid
		}, 
		
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then((response, activitycategoryid, communityid) =>{
				this.props.doLoading();
                console.log(response.data);
				let tmp = [];
				if(response.data.record.activitypic !== ""){
					tmp.push(response.data.record.activitypic);
				}
				
				this.setState({activityid : response.data.record.activityid});
				this.setState({activityname : response.data.record.activityname});
				this.setState({price : response.data.record.price});
				this.setState({activitycategoryid : response.data.record.activitycategoryid});
				this.setState({shortdesc : response.data.record.shortdesc});
				this.setState({fulldesc : response.data.record.fulldesc});
				this.setState({activitypic : tmp});
                ...
                <div className="box-container">
					<table>
						<tr>
                            ...
                            <td><Input type="text" name="activityname" id="activityname" placeholder="Activity Name" value={this.state.activityname} onChange = {(event) => this.setState({ activityname : event.target.value }) }/></td>
                        </tr>
						<tr>
						&nbsp;
						</tr>
						...
						<tr>
                            ...
                            <td><Input type="textarea" name="shortdesc" id="shortdesc" placeholder="Activity Short Description" value={this.state.shortdesc} onChange = {(event) => this.setState({ shortdesc : event.target.value }) }/></td>
                        </tr>
						<tr>
						&nbsp;
						</tr>
						<tr>
                            ...
                            <td><Input type="textarea" name="fulldesc" id="fulldesc" placeholder="Activity Full Description" value={this.state.fulldesc} onChange = {(event) => this.setState({ fulldesc : event.target.value }) }/></td>
                        </tr>
						<tr>
						&nbsp;
						</tr>
