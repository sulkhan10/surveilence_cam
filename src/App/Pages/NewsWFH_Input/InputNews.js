...
class InputNewsWFH extends Component {
    constructor(props) {
		super(props);
		this.globallang = getLanguage(activeLanguage, 'global');
		this.language = getLanguage(activeLanguage, 'inputwfh');
		this.state = {
			newsid: 0,
			title: '',
			newscategoryid: '', 
	...
	checkData = () => {
		...
		if(headline === null || headline === '' || newscategoryid === null || newscategoryid === 0 || communityid === null || communityid === 0 || previewmode === 0 || status === 0 || stickystatus === 0 || tabaction === 0){
			alert(this.language.validation);
			return false;
		}
	...
	onSubmit = () => {
        ...
        axios.post(serverUrl+'news_wfh_insert_update.php', body, 
		{headers: { 'Content-Type': 'application/x-www-form-urlencoded;charset=UTF-8' }})
            .then( (response) =>{
				this.props.doLoading();
				alert(this.language.savesuccess);
				this.props.history.push('/panel/listnewswfh');
            })
            .catch( (error) =>{
				this.props.doLoading();
                console.log(error);
