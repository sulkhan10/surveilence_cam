...
export default class UnitInvoiceEdit extends Component {
  constructor(props) {
    super(props);
    this.state = {
      idInv: props.match.params.invid,
      UnitShow: [],
      DebtorAcct: "",
      PeriodeMonth: "",
      PeriodeMonthShow: [
  ...
  doGetIdInvoice = () => {
    ...
    axios
      .post(
        serverUrl + "unit_inv_get_by_id.php",
        {
          invId: this.state.idInv,
        },
      ...
      .then((response) => {
        ...
        let debtor = this.state.UnitShow.filter(
          (dbtor) => dbtor.value === response.data.record.DebtorAcct
        );
        // console.log(debtor);
        this.setState({ DebtorAcct: debtor[0] });

        this.setState({ idInv: response.data.record.invId });

        this.setState({ PeriodeMonth: response.data.record.PeriodeMonth });
        this.setState({ PeriodeYear: response.data.record.PeriodeYear });
        this.setState({ Periode: response.data.record.Periode });
        this.setState({ Category: response.data.record.Category });
        this.setState({ InvNo: response.data.record.InvNo });
        this.setState({ InvAmt: response.data.record.InvAmt });
        this.setState({ InvDate: response.data.record.InvDate });
      })
      .catch((error) => {
        this.props.doLoading();
        alert(error);
