...
class ListEntertainment extends Component {
    constructor(props) {
		...
		this.language = getLanguage(activeLanguage, 'listentertainment');
	...
	doRowEdit = (row) => {
        console.log(row);
		this.props.history.push('/panel/editentertainment/'+row.entertainmentid);
    }
    ...
    doRowDelete = (row) => {
		...
		confirmAlert({
		  ...
		  buttons: [
			{
				label: 'Yes',
				onClick: (entertainmentid) => {
					var entertainmentid = row.entertainmentid;
					console.log(entertainmentid);
					this.deleteEntertainment(entertainmentid)					
			  }
			},
			{
			  label: 'No',
	...
	addNew = () => {
		this.props.history.push('/panel/inputentertainment');
	}
	
	doSearch = () => {
		this.props.doLoading();
		axios.post(serverUrl+'entertainment_list.php', {
	...
	deleteEntertainment = (entertainmentid) => {
		this.props.doLoading();
		axios.post(serverUrl+'entertainment_delete.php', {
